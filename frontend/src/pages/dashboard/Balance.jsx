...
export default function Balance() {
  ...
  return (
    <div data-testid="balance-page">
      <div className="mb-8">
        <div className="font-mono text-xs uppercase tracking-widest text-text-muted mb-1">// balance</div>
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        <div className="card-base p-6 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="font-mono text-xs uppercase tracking-widest text-text-muted mb-2 flex items-center gap-2">
        ...
        <div className="card-base p-6">
          <div className="font-mono text-xs uppercase tracking-widest text-text-muted mb-2">Pending</div>
          <div className="font-display text-4xl font-bold tracking-tight">
        ...
        <div className="card-base p-6">
          <div className="font-mono text-xs uppercase tracking-widest text-text-muted mb-2">Next payout</div>
      ...
      <div className="card-base overflow-hidden">
        <div className="px-5 py-4 border-b border-[var(--border)]">
          <div className="font-mono text-xs uppercase tracking-widest text-text-muted">Ledger</div>
        </div>
        <table className="w-full text-sm">
          ...
          <tbody>
            {ledger.length === 0 && (
              <tr><td colSpan={6} className="text-center text-text-muted py-10 text-sm">No ledger entries yet.</td></tr>
            )}
            {ledger.map((l) => (
              <tr key={l.id} className="border-t border-[var(--border)] hover:bg-surface-elevated/50" data-testid={`ledger-row-${l.id}`}>
                <td className="px-5 py-3">
                  <span className={`font-mono text-xs uppercase tracking-widest inline-flex items-center gap-1 ${l.type === "refund" ? "text-amber-400" : "text-emerald-400"}`}>
                ...
                <td className="px-5 py-3 text-right font-mono">{formatCents(l.gross || 0, l.currency)}</td>
                <td className="px-5 py-3 text-right font-mono text-text-muted">{formatCents(l.fee || 0, l.currency)}</td>
